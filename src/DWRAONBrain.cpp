...
//            if (randf(0,1)<0.05) boxes[i].id[j]=4;
            ...
            if (i < BRAINSIZE / 2)
            {
                boxes[i].id[j] = randi(0, INPUT_COUNT);
            }
        }
    }
...
DWRAONBrain::DWRAONBrain(const DWRAONBrain &other) : boxes(other.boxes)
{
}
